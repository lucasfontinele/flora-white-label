...
export interface AddStockDependencies {
...
export class AddStockUseCase {
  constructor(private readonly deps: AddStockDependencies) {}

  async execute(input: AddStockInput): Promise<InventoryItemReadModel> {
    return this.deps.unitOfWork.execute(async () => {
      ...
      item.addStock(input.quantity, input.reason);
