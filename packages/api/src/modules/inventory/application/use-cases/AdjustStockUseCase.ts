...
export class AdjustStockUseCase {
  constructor(private readonly deps: AdjustStockDependencies) {}

  async execute(input: AdjustStockInput): Promise<InventoryItemReadModel> {
    return this.deps.unitOfWork.execute(async () => {
      ...
      item.adjustStock(input.quantity, input.reason);
