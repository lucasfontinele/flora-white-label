...
export class ReleaseReservationUseCase {
  constructor(private readonly deps: ReleaseReservationDependencies) {}

  async execute(input: ReleaseReservationInput): Promise<InventoryItemReadModel> {
    return this.deps.unitOfWork.execute(async () => {
      ...
      item.releaseReservation(input.quantity, input.reason);
